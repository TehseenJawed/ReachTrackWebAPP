...
function ShowGeofenceArea({ rows, header, tablePaper, actionJSX, filterByGroup, filterByPlate, filterByModel, filterByMeter, filterByDriverName, filterByDateOfJoining, setLoading, editId, editStates }: TableProps) {
    const [selected, setSelected] = React.useState<any>([]);
    // Avoid a layout jump when reaching the last page with empty rows.
console.log('wwwwww.',rows);
    ...
    const showGroupsJSX = (value, key) => (
        <div>
            <div key={key} className='flex justify-between items-center py-4'>
                ...
                <div className='flex justify-center items-center w-3/12'>{value?.vehicles?.length || 0} Vehicles</div>
                <div className='flex justify-center items-center w-3/12'>{value?.alerts?.toString()}</div>
                <div className='flex justify-center items-center w-3/12'>{value?.description}</div>
                {actionJSX(value)}
            </div>
        </div>
    )
...
ShowGeofenceArea.defaultProps = defaultProps;

export default ShowGeofenceArea;