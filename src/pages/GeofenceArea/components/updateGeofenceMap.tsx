...
export default function UpdateGeofenceMap({ geofenceData, setGeofenceData, toUpdate = false }) {
    ...
    const renderPolygonShapes = useCallback(
        () =>
            (
                geofenceData.type === 'POLYGON' ? (
                    <div>
                        <Polygon options={{
                            ...
                            fillOpacity: 0.2,
                            editable: true
                        }}  path={geofenceData?.path.map(latLng => latLng)} />
                    </div>
                ) : null
            ),
        [geofenceData?.path]
