...
export default function AddVehicle({ open, setOpen }) {
    ...
    const handleSubmit = () => {
        if (vehicleData?.tracker_id && vehicleData?.number_plate && vehicleData?.make && vehicleData?.model && vehicleData?.year && vehicleData?.color && vehicleData?.odo_meter_reading && vehicleData?.type && vehicleData?.category && vehicleData?.shift_hours && vehicleData?.shift_time) {
            const vehicle = {
                ...vehicleData,
                group_id: group?.id,
                shift_hours: parseInt(vehicleData?.shift_hours),
    ...
    const contentJSX = () => {
        return (
            <div className='flex flex-wrap w-full'>
                ...
                <div className={`${group?.length ? "h-[120px]" : "mb-3"} w-full overflow-auto`}>
                    <GroupTree group={group} setGroup={setGroup} placeholder={"Select Parent Group"} />
                    {
                        group?.length ? "" : "No Group is available"
                    }
                </div>
                <FormControl sx={{ m: 0, minWidth: '100%' }} size="small">
                    <InputLabel className='bg-white pr-2' id="demo-select-small-label">Select Tracker</InputLabel>
                    <Select
                ...
                <InputField className="w-full mt-4" type='number' fieldClassName="w-full" label={"SHIFT HOURS*"} error={error ? vehicleData?.shift_hours == "" : false} placeholder={"Shift Hours"} value={vehicleData?.shift_hours} setChange={(e) => setVehicleData({ ...vehicleData, shift_hours: e.target.value })} />
            ...
            >
                ...
                <DialogActions>
                    <div className='w-full flex justify-end'>
                        ...
                        <Button className='ml-4' onClick={handleSubmit}>CREATE</Button>
                    </div>
                </DialogActions>
            </Dialog>
        </React.Fragment>
